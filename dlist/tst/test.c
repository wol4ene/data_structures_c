...
/**
 * Helper to verify an dlist is as expected
 *
 * If just testing for an empty list (exp_count == 0), the last
 * two args, exp_data and pos, will be ignored
 *
 * @param p (i) opaque pointer to dlist
 * @param exp_count (i) expected num nodes in dlist
 ...
 */
static int 
_dlist_verify(DListPtr p, int exp_count, int exp_data, int pos)
{
    int passed = 1;
    int cur_count = dlist_count(p);
    ...
    if (exp_count > 0) {
	int cur_data = *(int*)dlist_get_pos(p, pos);
...
out:
    if (passed) {
        logger(dbgInfo, "Verified list has %i nodes, and data %i at pos %i",
                exp_count, exp_data, pos);
    }
    return passed;
}
...
test1(const char *test_name) {
    ...
    DListPtr p = dlist_new(test_name);
    if (0 != dlist_count(p)) {
...
test2(const char *test_name) {
    ...
    dlist_add_tail(p, &data0);
    if (1 != _dlist_verify(p, 1, data0, 0)) {
	FAIL_TEST;
    }

    /* tail add another node, verify count and data are expected */
    dlist_add_tail(p, &data1);
    if (1 != _dlist_verify(p, 2, data1, 1)) {
	FAIL_TEST;
    }
...
test3(const char *test_name) {
    ...
    dlist_add_head(p, &data0);
    if (1 != _dlist_verify(p, 1, data0, 0)) {
	FAIL_TEST;
    }

    /* head add another node, verify count and data are expected */
    dlist_add_head(p, &data1);
    if (1 != _dlist_verify(p, 2, data1, 0)) {
	FAIL_TEST;
    }

    /* make sure data0 is now moved to pos 1*/
    if (1 != _dlist_verify(p, 2, data0, 1)) {
	FAIL_TEST;
    }
...
test4(const char *test_name) {
    ...
    dlist_add_head(p, &data);
    if (1 != _dlist_verify(p, 1, data, 0)) {
	FAIL_TEST;
    }
    dlist_del_head(p);
    if (1 != _dlist_verify(p, 0, 0, 0)) {
	FAIL_TEST;
    }
...
test5(const char *test_name) {
	...
	dlist_add_tail(p, &arr[i]);
    }

    /* verify each node is in its expected spot */
    for (i = 0; i < num_nodes; i++) {
	if (1 != _dlist_verify(p, 9, arr[i], i)) {
	    FAIL_TEST;
	}
    }
...
test6(const char *test_name) {
     ...
     */
    for (i = 0; i < num_nodes; i++) {
	if (1 != _dlist_verify(p, 9, arr[num_nodes-1-i], i)) {
	    FAIL_TEST;
	}
    }
...
test7(const char *test_name) {
    ...
    dlist_apply_fn(p, _dlist_test_add_two);

    /* now verify each node is +2 of the original */
    for (i = 0; i < num_nodes; i++) {
	if (1 != _dlist_verify(p, 9, (orig[i]+2), i)) {
	    FAIL_TEST;
	}
    }
